...
{{prodname}} supports both host-to-host encryption for pod traffic and direct node-to-node communication. Because {{prodname}} is not implemented using a sidecar, traffic is not encrypted for the full journey from one pod to another; traffic is only encrypted on the host-to-host portion of the journey.
...
For OpenShift, add the Felix configuration with WireGuard enabled [under custom resources](../getting-started/install-on-clusters/openshift/installation/index.mdx#provide-additional-configuration).
...
![Wireguard Dashboard Toggle](/img/calico-enterprise/3.14/wireguard/stats-toggle.png)
...
- [Secure {{prodname}} component communications](../operations/comms/index.mdx)
