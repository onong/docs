...
Configure an external identity provider (IdP), create a user, and log in to {{prodname}} Manager UI.
...
- [Configure access to Manager UI](access-the-manager.mdx)
...
1. Create values for some required variables. `MANAGER_URL` is the URL where {{prodname}} Manager will be accessed,
...
Most IdPs require redirect URIs to be allowed to redirect users at the end of the OAuth flow to the {{prodname}} Manager or to Kibana. Consult your IdP documentation for authorizing your domain for the respective origins and destinations.
...
- If logging into Kibana fails with a `cookie not present` error, update the browser settings to allow third-party cookies, as {{prodname}} Manager UI uses Kibana's cookies during login.
