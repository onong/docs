...
export const version = '{{version}}' === 'master' ? 'master' : '{{releases.0.title}}';
...
<CodeBlock language='batch'>
  {'{{version}}' === 'master'
    ? `curl -o calicoctl -O -L {{filesUrl}}/download/binaries/${version}/calicoctl`
    : `curl -o calicoctl -O -L {{downloadsurl}}/ee/binaries/${version}/calicoctl`}
</CodeBlock>
...
<CodeBlock language='batch'>
  {'{{version}}' === 'master'
    ? `curl -o calicoctl -O -L {{filesUrl}}/download/binaries/${version}/calicoctl-darwin-amd64`
    : `curl -o calicoctl -O -L {{downloadsurl}}/ee/binaries/${version}/calicoctl-darwin-amd64`}
</CodeBlock>
...
If you get the error, `cannot be opened because the developer cannot be verified` when using `caicoctl` for the first time, go to `Applicaitons > System Prefences > Security & Privacy` in the `General` tab at the bottom of the window click `Allow anyway`.
...
<CodeBlock language='batch'>
  {'{{version}}' === 'master'
    ? `curl -o kubectl-calico -O -L {{filesUrl}}/download/binaries/${version}/calicoctl`
    : `curl -o kubectl-calico -O -L {{downloadsurl}}/ee/binaries/${version}/calicoctl`}
</CodeBlock>
...
<CodeBlock language='batch'>
  {'{{version}}' === 'master'
    ? `curl -o kubectl-calico -O -L {{filesUrl}}/download/binaries/${version}/calicoctl-darwin-amd64`
    : `curl -o kubectl-calico -O -L {{downloadsurl}}/ee/binaries/${version}/calicoctl-darwin-amd64`}
</CodeBlock>
...
If you get the error, "cannot be opened because the developer cannot be verified" when using `calicoctl` for the first time, go to Applications > System Prefences > Security & Privacy in the **General** tab at the bottom of the window click `Allow anyway`.
...
<CodeBlock language='batch'>
  {'{{version}}' === 'master'
    ? `Invoke-WebRequest -Uri "{{filesUrl}}/download/binaries/${version}/calicoctl-windows-amd64.exe" -OutFile "kubectl-calico.exe"`
    : `Invoke-WebRequest -Uri "{{downloadsurl}}/ee/binaries/${version}/calicoctl-windows-amd64.exe" -OutFile "kubectl-calico.exe"`}
</CodeBlock>
...
1. Confirm that the image has loaded by typing `docker images`.
   {%- assign c = releases.0.components.calicoctl %}
...
```batch
kubectl apply -f {{filesUrl}}/manifests/calicoctl.yaml
```

:::note

You can also [view the YAML in a new tab](/manifests/calicoctl.yaml).
...
In order to use the `calicoctl` alias when reading manifests, redirect the file into stdin, for example: `batch calicoctl create -f - < my_manifest.yaml.`
