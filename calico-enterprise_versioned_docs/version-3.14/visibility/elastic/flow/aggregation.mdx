...
Beyond using filtering to suppress flow logs, {{prodname}} provides controls to aggregate flow logs. Although aggressive aggregation levels reduce flow volume and costs, it can also reduce visibility into specific metadata of allowed and denied traffic. Review this article to see which level of aggregation is suitable for your implementation.
...
- Workload endpoints with a similar prefix "access-6b687c8dcb-_" in the "policy-demo" namespace connected to a workload-endpoints/pods with prefix "nginx-86c57db685-_" exposing a service on port "80".
