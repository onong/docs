...
- Kubernetes in AWS only; for on-prem, see [this guide](egress-gateway-on-prem.mdx)
...
Because the encapsulated traffic from host to egress gateway is "opaque" to policy, there are effectively two places where
