...
```powershell
ipmo {{rootDirWindows}}\libs\hns\hns.psm1
Get-HNSNetwork
```
...
If the error includes `loading config file <path-to-kubeconfig\>`, follow the instructions in [Set environment variables](kubernetes/standard.mdx#install-calico-enterprise-for-windows-and-kubernetes-on-windows-nodes) to update the `KUBECONFIG` environment variable to the path of your kubeconfig file.
