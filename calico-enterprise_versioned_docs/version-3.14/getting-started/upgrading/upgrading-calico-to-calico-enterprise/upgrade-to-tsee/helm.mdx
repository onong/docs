...
1. Install the {{prodname}} custom resource definitions.
   ...
   kubectl apply -f {{filesUrl}}/manifests/prometheus-operator-crds.yaml
   kubectl apply -f {{filesUrl}}/manifests/eck-operator-crds.yaml
   ```
...
1. Apply the following manifest to secure {{prodname}} with network policy:
