...
Configure {{prodname}} AWS security groups integration.

## Value

AWS security group integration for {{prodname}} allows you to combine AWS security groups with network policy to enforce granular access control between Kubernetes pods and AWS VPC resources.
...
- Helm install
- Multiple VPCs
- {{prodname}} non-cluster hosts, and auto-hostendpoint features. Alternatives to secure host endpoints:
  ...
  - Use {{prodname}} [GlobalNetworkSets](../../../reference/resources/globalnetworkset.mdx) and use custom labels for selection in policy.
...
- Networking provider is [Amazon VPC Networking](../../../reference/public-cloud/aws.mdx#using-aws-networking).
...
- You have installed [{{prodname}} for EKS](../../../getting-started/install-on-clusters/eks.mdx)
...
- You are not using the {{prodname}} [auto hostendpoints feature](../../hosts/kubernetes-nodes.mdx), and have not created any [host endpoints](../../../reference/resources/hostendpoint.mdx) that have a `spec.node` value that matches any of your Kubernetes nodes.

  Verify that there are no entries for {{prodname}} host endpoints using this command:
...
- [jq commands](https://stedolan.github.io/jq/)
...
The following commands gather the required information of a particular EKS cluster with name `$CLUSTER_NAME` that was created with [`eksctl`](https://github.com/weaveworks/eksctl):
...
> **Note**: Commands above apply only to EKS clusters with unmanaged nodegroups i.e. [eksctl without --managed](https://eksctl.io/usage/eks-managed-nodes/) option.
...
1.  Install AWS per-account resources.
    ...
    --template-body {{filesUrl}}/manifests/aws/security-group-integration/account-cf.yaml
...
1.  Install AWS per-VPC resources.
    ...
    --template-body {{filesUrl}}/manifests/aws/security-group-integration/vpc-cf.yaml
...
1.  Install AWS per-cluster resources.
    ...
    --template-body {{filesUrl}}/manifests/aws/security-group-integration/cluster-cf.yaml
...
- [Configure fine-grained access control between VPC endpoints and pods](tiers-and-policy.mdx)
