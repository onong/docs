...
A _workload_ is a container or VM that {{prodname}} handles the virtual networking for. In Kubernetes, workloads are pods.
A **workload endpoint** is the virtual network interface a workload uses to connect to the {{prodname}} network.

**IP pools** are ranges of IP addresses that {{prodname}} uses for **workload endpoints**.
...
Let's define two IP pools for use in this cluster. You can have a production-ready {{prodname}} install with only a single
...
In this second pool, we set `disabled` to `true`, meaning that {{prodname}} will not create new pods with addresses in the pool
but will still recognize pods with these addresses as part of the {{prodname}} network. Later, in the
[test networking](test-networking.mdx) lab, we will enable this pool and demonstrate how to control which pools your pods are assigned
addresses from.
...
[Install CNI plugin](install-cni-plugin.mdx)
