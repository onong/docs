...
> **Note**: {{prodname}} makes use of the Kubernetes Container Storage Interface (CSI) to support various types of volumes. The necessary drivers required for CSI
...
1. Install the {{prodname}} manifest.
   ...
   ```batch
   kubectl apply -f {{releases.0.manifests_url}}/manifests/calico-vxlan.yaml
   ```
