...
```batch
kubectl create -f {{releases.0.manifests_url}}/manifests/custom-resources.yaml
```
...
```batch
kubectl apply -f {{releases.0.manifests_url}}/manifests/calico.yaml
```

:::note

You can also
[view the YAML in a new tab]({{releases.0.manifests_url}}/manifests/calico.yaml).
