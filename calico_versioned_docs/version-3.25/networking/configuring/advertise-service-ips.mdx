...
1. Configure MetalLB to provision addresses by creating the following config map, replacing `x.x.x.x/16` with the CIDR given to {{prodname}} in the steps above.
...
- [Apply policy to services exposed externally as cluster IPs](../../network-policy/services/services-cluster-ips.mdx)
- [Apply policy to Kubernetes node ports](../../network-policy/services/kubernetes-node-ports.mdx)
