...
This policy has no `order` field specified which causes it to default
to the highest value. Because higher order values have the lowest order of precedence,
{{prodname}} will apply this policy after all other policies. Refer to the
[policy spec](../resources/networkpolicy.mdx#spec) for
more discussion.
