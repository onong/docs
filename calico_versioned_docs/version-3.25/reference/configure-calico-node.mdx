...
| ETCD_KEY_FILE      | Path to the file containing the private key matching the `{{nodecontainer}}` client certificate. Enables `{{nodecontainer}}` to participate in mutual TLS authentication and identify itself to the etcd server. Example: `/etc/node/key.pem` (optional)                                                                                                 | string |
| ETCD_CERT_FILE     | Path to the file containing the client certificate issued to `{{nodecontainer}}`. Enables `{{nodecontainer}}` to participate in mutual TLS authentication and identify itself to the etcd server. Example: `/etc/node/cert.pem` (optional)                                                                                                               | string |
| ETCD_CA_CERT_FILE  | Path to the file containing the root certificate of the certificate authority (CA) that issued the etcd server certificate. Configures `{{nodecontainer}}` to trust the CA that signed the root certificate. The file may contain multiple root certificates, causing `{{nodecontainer}}` to trust each of the CAs included. Example: `/etc/node/ca.pem` | string |
...
in the cluster, see [decommissioning a node](../operations/decommissioning-a-node.mdx).
