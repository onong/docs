...
> **Important**: Do not use a custom `blockSize` until **all** {{prodname}} components have been updated to a version that
...
{{prodname}} supports Kubernetes [annotations that force the use of specific IP addresses](../configure-cni-plugins.mdx#requesting-a-specific-ip-address). These annotations take precedence over the `allowedUses` field.
...
> **Tip**: To prevent an IP pool from being used automatically by {{prodname}} IPAM, while still allowing
