...
![Detail of BPF program showing that packets are sent to a separate (generated) policy program,](/img/calico/3.25/bpf-policy.svg 'Expanded view of tc program showing policy.')
...
![Diagram showing BPF program attached to socket connect call; it does NAT at connect time,](/img/calico/3.25/bpf-connect-time.svg 'BPF program attached to socket connect call.')
