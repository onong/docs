...
```batch
docker pull {{registry}}{{imageNames.calicoctl}}:{{releases.0.title}}
```
...
- **etcd**
  ...
  ```batch
  kubectl apply -f {{releases.0.manifests_url}}/manifests/calicoctl-etcd.yaml
  ```

  :::note

  You can also
  [view the YAML in a new tab]({{releases.0.manifests_url}}/manifests/calicoctl-etcd.yaml).
...
- **Kubernetes API datastore**
  ...
  ```batch
  kubectl apply -f {{releases.0.manifests_url}}/manifests/calicoctl.yaml
  ```

  :::note

  You can also
  [view the YAML in a new tab]({{releases.0.manifests_url}}/manifests/calicoctl.yaml).
