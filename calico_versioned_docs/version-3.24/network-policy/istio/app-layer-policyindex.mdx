...
```batch
kubectl apply -f {{releases.0.manifests_url}}/manifests/csi-driver.yaml
```
...
1. Install Istio using [installation guide in the project documentation](https://istio.io/v1.9/docs/setup/install/).
...
Next, create the following [PeerAuthentication](https://istio.io/latest/docs/reference/config/security/peer_authentication/) policy.
...
1. Follow the [Automatic sidecar injection instructions](https://archive.istio.io/v1.9/docs/setup/additional-setup/sidecar-injection/#automatic-sidecar-injection) to install the sidecar injector and enable it in your chosen namespace(s).
1. Patch the istio-sidecar-injector `ConfigMap` to enable injection of Dikastes alongside Envoy.

<Tabs>
<TabItem label="Istio v1.10.x" value="Istio v1.10.x-0">

```batch
curl {{releases.0.manifests_url}}/manifests/alp/istio-inject-configmap-1.10.yaml -o istio-inject-configmap.yaml
kubectl patch configmap -n istio-system istio-sidecar-injector --patch "$(cat istio-inject-configmap.yaml)"
```

[View sample manifest]({{releases.0.manifests_url}}/manifests/alp/istio-inject-configmap-1.10.yaml)

</TabItem>
<TabItem label="Istio v1.9.x" value="Istio v1.9.x-1">

```batch
curl {{releases.0.manifests_url}}/manifests/alp/istio-inject-configmap-1.9.yaml -o istio-inject-configmap.yaml
kubectl patch configmap -n istio-system istio-sidecar-injector --patch "$(cat istio-inject-configmap.yaml)"
```

[View sample manifest]({{releases.0.manifests_url}}/manifests/alp/istio-inject-configmap-1.9.yaml)
...
<Tabs>
<TabItem label="Istio v1.10.x and v1.9.x" value="Istio v1.10.x and v1.9.x-2">

```batch
kubectl apply -f {{releases.0.manifests_url}}/manifests/alp/istio-app-layer-policy-envoy-v3.yaml
```

[View sample manifest]({{releases.0.manifests_url}}/manifests/alp/istio-app-layer-policy-envoy-v3.yaml)
...
Envoy must be able to communicate with the `istio-pilot.istio-system service`. If you apply any egress policies to your pods, you _must_ enable access. For example, you could [apply a network policy]({{filesUrl}}/security/calico-network-policy).

```batch
kubectl apply -f {{filesUrl}}/getting-started/kubernetes/installation/manifests/app-layer-policy/allow-istio-pilot.yaml
```
