...
The the ipPools array can take at most one IPv4 and one IPv6 CIDR, and only takes effect when installing {{prodname}} for the first
...
Refer to the
[Calico ConfigMap manifest]({{releases.0.manifests_url}}/manifests/alp/istio-inject-configmap-1.4.2.yaml) for an
