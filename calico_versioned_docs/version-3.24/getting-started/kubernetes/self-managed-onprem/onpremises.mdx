...
1. First, install the operator on your cluster.
   ...
   ```
   kubectl create -f {{releases.0.manifests_url}}/manifests/tigera-operator.yaml
   ```

1. Download the custom resources necessary to configure {{prodname}}

   ```
   curl {{releases.0.manifests_url}}/manifests/custom-resources.yaml -O
   ```
...
1. Download the {{prodname}} networking manifest for the Kubernetes API datastore.
   ...
   ```batch
   curl {{releases.0.manifests_url}}/manifests/calico-typha.yaml -o calico.yaml
   ```
...
1. Download the {{prodname}} networking manifest for etcd.
   ...
   ```batch
   curl {{releases.0.manifests_url}}/manifests/calico-etcd.yaml -o calico.yaml
   ```
