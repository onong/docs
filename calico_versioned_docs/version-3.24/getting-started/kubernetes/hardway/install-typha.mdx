...
It watches the Kubernetes resources and {{prodname}} custom resources used by these daemons, and whenever a resource changes
...
Define a cluster role for Typha with permission to watch {{prodname}} datastore objects.
...
[Install calico/node](install-node.mdx)
