...
1. Install the helm chart in the `tigera-operator` namespace.
   ...
   ```
   helm install {{prodnamedash}} projectcalico/tigera-operator --version {{releases.0.title}} --namespace tigera-operator
   ```
...
1. Download the {{version}} operator manifest.
   ...
   ```batch
   curl {{releases.0.manifests_url}}/manifests/tigera-operator.yaml -O
   ```
...
1. Download the {{version}} manifest that corresponds to your original installation method.
   ...
   **{{prodname}} for policy and networking**
   ...
   ```batch
   curl {{releases.0.manifests_url}}/manifests/calico.yaml -O
   ```

   **{{prodname}} for policy and flannel for networking**

   ```batch
   curl {{releases.0.manifests_url}}/manifests/canal.yaml -O
   ```

   **{{prodname}} for policy (advanced)**

   ```batch
   curl {{releases.0.manifests_url}}/manifests/calico-policy-only.yaml -O
   ```
...
1. If you have [enable application layer policy](../../network-policy/istio/app-layer-policyindex.mdx),
...
1. Download the {{version}} manifest that corresponds to your original installation method.
   ...
   **{{prodname}} for policy and networking**
   ...
   ```batch
   curl {{releases.0.manifests_url}}/manifests/calico-etcd.yaml -O
   ```

   **{{prodname}} for policy and flannel for networking**

   ```batch
   curl {{releases.0.manifests_url}}/manifests/canal-etcd.yaml -O
   ```
...
1. If you have [enabled application layer policy](../../network-policy/istio/app-layer-policyindex.mdx),
   follow [the instructions below](#upgrading-if-you-have-application-layer-policy-enabled) to complete your upgrade. Skip this if you are not using Istio with {{prodname}}.
...
If you have [enabled application layer policy](../../network-policy/istio/app-layer-policyindex.mdx),
...
1. Update the Istio sidecar injector template to use the new version of Dikastes. Replace `<your Istio version>` below with
   ...
   ```batch
   kubectl apply -f {{releases.0.manifests_url}}/manifests/alp/istio-inject-configmap-<your Istio version>.yaml
   ```
