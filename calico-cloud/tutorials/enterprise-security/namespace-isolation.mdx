...
- The security tier will delegate (pass) microsegementation controls to the application tier
- We will use a GlobalNetworkPolicy with cluster wide scope that applies to pods in all namespaces in the cluster.

For example:

![multi-tenancy-ingress](/img/calico-cloud/master/multi-tenancy-ingress.png)
