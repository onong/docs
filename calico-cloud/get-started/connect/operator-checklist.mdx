---
title: Tigera operator troubleshooting checklist
...
The cluster you are using to install {{prodname}} does not have a CNI plugin installed, the CNI is incompatible. If your cluster has functional pod networking and you see this message, it is likely that kubelet has been configured to use kubenet networking, which is not compatible with {{prodname}}. You can use a different cluster, or re-create your cluster with [compatible networking](requirements/system-requirements.mdx).
...
| 3   | compliance                    | TRUE      | FALSE       | FALSE    | 9m39s |
| 4   | intrusion-detecton            | TRUE      | FALSE       | FALSE    | 9m49s |
| 5   | log-collector                 | TRUE      | FALSE       | FALSE    | 9m29s |
| 6   | management-cluster-connection | TRUE      | FALSE       | FALSE    | 9m54s |
...
**6 - management-cluster-connection**
...
`FelixConfiguration` contains configuration that are not configured as environment variables to the `alico-node` container.
...
![contact-support](/img/calico-cloud/master/contact-support.png)
