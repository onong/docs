...
Honeypods can optionally be monitored using a {{prodname}} controller that periodically polls selected honeypods for suspicious activity and scans its traffic. Alerts are generated in the Events tab of {{prodname}} Manager UI.
...
```batch
kubectl apply -f {{filesUrl_CE}}/manifests/threatdef/honeypod/controller.yaml
```

For OpenShift deployments, the controller requires privileged access:

```batch
kubectl apply -f {{filesUrl_CE}}/manifests/threatdef/honeypod/controller_os.yaml
```

### Add custom Snort rules

You can add custom Snort rules to the controller using a ConfigMap. By default, {{prodname}} uses the [Snort Community Ruleset](https://www.snort.org/downloads/#rule-downloads).
