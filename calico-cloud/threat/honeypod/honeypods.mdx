...
Honeypods can be configured on a per-cluster basis using "template" honeypod manifests that are easily customizable. Any alerts triggered are displayed in the Alerts tab in {{prodname}} Manager UI. The Honeypod Dashboard in Kibana provides an easy way to monitor and analyze traffic reaching the honeypods.
...
```batch
kubectl apply -f {{filesUrl_CE}}/manifests/threatdef/honeypod/ip-enum.yaml
```
...
```batch
kubectl apply -f {{filesUrl_CE}}/manifests/threatdef/honeypod/expose-svc.yaml
```
...
```batch
kubectl apply -f {{filesUrl_CE}}/manifests/threatdef/honeypod/vuln-svc.yaml
```
...
- [Monitor honeypods](honeypod-controller.mdx)
