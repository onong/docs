...
CIS benchmarks runs only on nodes where {{prodname}} is running. This limitation may exclude master nodes in some managed cloud platforms (AKS, EKS, GKE). Because the user has limited control over installation of master nodes in managed cloud platforms, these reports may have limited use for cloud users.
...
1. Download the pod template corresponding to your installation method.  
   **Operator**
   ...
   ```batch
   curl -O {{filesUrl_CE}}/manifests/compliance-reporter-pod.yaml
   ```
