...
allowed by the [failsafe rules](../../reference/host-endpoints/failsafe.mdx). Because there is no default-deny
...
can successfully execute commands like `kubectl get hostendpoints.p` and `kubectl get globalnetworkpolicy.p`—but that it is impossible to connect into the cluster from outside
(except for any [failsafe rules](../../reference/host-endpoints/failsafe.mdx).
For example, if the
