...
The procedure for configuring NetworkManager to ignore {{prodname}} interfaces
...
1. Check the interfaces that NetworkManager ignores.
   ...
   If this does not to prevent NetworkManager from interfering with {{prodname}} networking, try disabling NetworkManager. If disabling NetworkManager does not stop it from interfering with {{prodname}} networking, you may need to remove NetworkManager. This will require manual network configuration.
...
Also be aware that connection information can be specified as a config file rather than using environment variables. See [Installing calicoctl](../clis/calicoctl/install.mdx)
for details.
...
For anomaly detection jobs to start, the Elasticsearch cluster requires sufficient available memory. See Elasticsearch [sizing guidelines](../logstorage/log-storage-recommendations.mdx).
